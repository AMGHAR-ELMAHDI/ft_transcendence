...
function register() {
	...
	useEffect(() => {
		...
		const content = document.querySelector('.register') as HTMLElement;
	
		// saving the client without 42 auth
		login?.addEventListener('click', () => {
			let CaseErr: boolean = false
			const inputs: NodeListOf<HTMLInputElement> = document?.querySelectorAll('.register input');
			...
			var headers = {
			  'firstname' : document?.querySelector<HTMLInputElement>('input[name="FirstName"]')?.value || '',
			  'lastname' : document?.querySelector<HTMLInputElement>('input[name="LastName"]')?.value || '',
			  'email' : document?.querySelector<HTMLInputElement>('input[name="emailIn"]')?.value || '',
			  'password' : document?.querySelector<HTMLInputElement>('input[name="password"]')?.value || ''
			}
			setTimeout(() => window.location.href = 'http://localhost:8000/AutoSave/?email=' + encodeURIComponent(headers.email) + "&lastname=" + encodeURIComponent(headers.lastname) + "&firstname=" + encodeURIComponent(headers.firstname) + "&password=" + encodeURIComponent(headers.password), 1000);
		});
	...
	return (
		<div className="content">
			<div className="register">
				...
				{
					error.length > 0
					?<div className="statusError">{error}</div>:''
				}
