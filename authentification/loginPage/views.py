...
def isUserExist(email):
	user_filter = Player.objects.filter(email__iexact=email)
...
def	FetchData(request):
	if request.method == 'GET':
		...
		if (resp2.status_code == 200):
			...
			item = Player(firstname=FirstName, lastname=LastName, email=email, username=username)
			item.save()
			request.session['response'] = json.dumps(data)
			return redirect('http://localhost:5173')
...
def Users(request):
	users = Player.objects.order_by('coins', 'firstname')
...
def checkme(request):
	...
	if data == None:
		...
		try:
			user_ = Player.objects.get(email=email)
...
def	SaveManual(request):
	if request.method == 'GET':
		...
		if (isUserExist(email)):
			...
			try:
				inst_ = Player.objects.create(firstname=FirstName, lastname=LastName, email=email, password=passwd)
				inst_.save()
				data = {
					'email' : email
				}
...
def	login(request):
	if request.method == 'GET':
		try:
			email = request.GET.get('email')
			passwd = request.GET.get('password')
			userInfo = Player.objects.get(email=email)
